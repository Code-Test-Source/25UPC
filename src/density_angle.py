...
class ArtillerySolver3D:
    def __init__(self):
        ...
        # Reference sea-level density (can be swept per-run). Actual density
        # used during simulation varies with altitude via get_air_density().
        self.air_density = 1.2             # kept for backward-compat; use sea_level_density for profiles
        self.sea_level_density = float(self.air_density)
        # Simple exponential scale-height model (rho(z) = rho0 * exp(-z/scale_height)).
        # Scale height ~8500 m is a reasonable approximation for troposphere.
        self.scale_height = 8500.0  # meters
        # Optional: user may assign a callable self.air_density_func(z) to override the simple model.
        self.air_density_func = None
    ...
    def get_air_density_at_height(self, z):
        """Return air density (kg/m^3) at altitude z (meters).

        By default this uses a simple exponential scale-height model anchored at
        self.sea_level_density. If self.air_density_func is set to a callable,
        that will be used instead (useful for high-fidelity profiles).
        """
        # ensure numeric
        try:
            zval = float(z)
        ...
        if self.air_density_func is not None:
            try:
                return float(self.air_density_func(zval))
            except Exception:
                # fallback to exponential model on error
                pass

        # clamp: do not allow extremely negative altitudes to blow up density
        if zval < 0.0:
            zval = 0.0

        rho0 = float(self.sea_level_density)
        H = max(100.0, float(self.scale_height))
        return rho0 * np.exp(-zval / H)

    def wind_force(self, velocity, wind_velocity, altitude=0.0):
        """Calculate aerodynamic wind/drag force on projectile at given altitude.

        Parameters:
        - velocity: projectile velocity vector (3,)
        - wind_velocity: air/wind velocity vector (3,)
        - altitude: altitude z (m) at which to evaluate air density
        """
        ...
        # Evaluate local air density at altitude
        rho_local = self.get_air_density_at_height(altitude)

        # Drag force magnitude (quadratic in speed)
        drag_magnitude = 0.5 * rho_local * cd * self.area * speed**2
    ...
    def projectile_dynamics(self, t, state, muzzle_velocity, elevation_angle, azimuth_angle, wind_velocity):
        ...
        # Use current altitude (state[2]) to evaluate local air density
        wind_force_vec = self.wind_force(velocity, wind_velocity, altitude=position[2])
    ...
    def firing_error(self, parameters, target_distance, initial_altitude, wind_velocity):
        ...
        if (muzzle_velocity < 100 or muzzle_velocity > self.max_muzzle_velocity or
            elevation_angle < 10 or elevation_angle > 25 or
    ...
    def direct_optimization(self, target_distance, initial_altitude, wind_velocity):
        ...
        initial_guesses = [
            ...
            [200, 20, 0],
            [300, 20, 3],
            [400, 20, -3],   # Medium velocity, medium angle, straight
            [350, 25, 2],   # Upper-bound angle candidate
        ]
    ...
    def find_candidates_coarse(self, target_distance, initial_altitude, wind_velocity,
        ...
        if azim_choices is None:
            ...
            if np.linalg.norm(wind_velocity) < 1e-1:
                ...
                azim_choices = [-10.0,-7.5, -5.0,-2.5, 0.0, 2.5, 5.0,7.5,10.0]
    ...
    def plot_trajectory_3d(self, muzzle_velocity, elevation_angle, azimuth_angle,
        ...
        for i in range(t_eval.size):
            ...
            # evaluate drag at the trajectory altitude z_traj[i]
            a_drag = self.wind_force(vel, wind_velocity, altitude=z_traj[i]) / self.mass
...
def main():
    ...
    # Generate firing table for practical use (expanded wind & density sweep)
    print("\nPRACTICAL FIRING TABLE (density sweep)")
    distances = [1000, 1100, 1200, 1300, 1400, 1500]

    # Wind setup: directions every 30 degrees (0..180) and speeds every 2.5 m/s from 0..10
    wind_dirs = list(range(0, 181, 30))  # 0,30,...,180
    wind_speeds = [round(2.5 * i, 1) for i in range(0, 5)]  # 0.0,2.5,5.0,7.5,10.0
    wind_conditions = [(s, d) for d in wind_dirs for s in wind_speeds]

    # Air density sweep: 1.1 .. 1.3 kg/m^3 (step 0.1)
    densities = [round(1.1 + 0.1 * i, 1) for i in range(0, 3)]  # 1.1,1.2,1.3

    # Ensure output directory exists
    out_dir = os.path.abspath('.')
    print(f"Writing CSVs and plots to: {out_dir}")

    # Store per-density summaries for a combined plot
    all_density_summaries = {}

    for rho in densities:
        artillery.air_density = float(rho)
        print(f"\nRunning firing table for air density = {rho:.1f} kg/m^3; wind cases per distance = {len(wind_conditions)}")

        firing_table = artillery.generate_firing_table(distances, wind_conditions)

        # Save CSV for this density
        csv_path = os.path.join(out_dir, f"firing_table_density_{rho:.1f}.csv")
        try:
            with open(csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['distance', 'wind_magnitude', 'wind_direction', 'muzzle_velocity', 'elevation', 'azimuth', 'error'])
                for row in firing_table:
                    writer.writerow([
                        row['distance'], row['wind_magnitude'], row['wind_direction'],
                        row['muzzle_velocity'], row['elevation'], row['azimuth'], row['error']
                    ])
        except Exception as e:
            print(f"Failed to write CSV {csv_path}: {e}")

        # Compute average muzzle velocity per distance for this density
        avg_vs = []
        for distance in distances:
            distance_data = [item for item in firing_table if item['distance'] == distance]
            if distance_data:
                avg_velocity = float(np.mean([item['muzzle_velocity'] for item in distance_data]))
            else:
                avg_velocity = float('nan')
            avg_vs.append(avg_velocity)

        all_density_summaries[rho] = avg_vs

        # Plot per-density summary
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(distances, avg_vs, marker='o')
        ax.set_title(f"Avg muzzle velocity vs distance (rho={rho:.1f} kg/m^3)")
        ax.set_xlabel("Distance (m)")
        ax.set_ylabel("Avg muzzle velocity (m/s)")
        ax.grid(True)
        png_path = os.path.join(out_dir, f"avg_velocity_rho_{rho:.1f}.png")
        try:
            fig.savefig(png_path)
            plt.close(fig)
            print(f"Saved {csv_path} and {png_path}")
        except Exception as e:
            print(f"Failed to save plot {png_path}: {e}")

    # Combined plot across densities
    try:
        fig, ax = plt.subplots(figsize=(10, 6))
        for rho in sorted(all_density_summaries.keys()):
            ax.plot(distances, all_density_summaries[rho], marker='o', label=f"rho={rho:.1f}")
        ax.set_title("Avg muzzle velocity vs distance for densities 1.1–1.3 kg/m^3")
        ax.set_xlabel("Distance (m)")
        ax.set_ylabel("Avg muzzle velocity (m/s)")
        ax.grid(True)
        ax.legend()
        combo_png = os.path.join(out_dir, "avg_velocity_all_densities.png")
        fig.savefig(combo_png)
        plt.show()
        print(f"Saved combined plot: {combo_png}")
    except Exception as e:
